...
// Spend per category in the CURRENT calendar month (for budgets).
export function monthlyCategorySpend(transactions) {
  const now = new Date();
  const txs = spendDebits(clean(transactions)).filter((t) => {
...
export function computeStats(transactions, opts = {}) {
  ...
  const investmentTotal = sum(debs.filter((t) => t.category === "Investment"));
  ...
  return {
    ...
    extraSavings,
    investmentTotal,
    healthScore: healthScore(transactions, extraSavings + investmentTotal),
  };
}
